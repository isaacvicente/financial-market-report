...
### Conclusão
Como pode ser analisado, o modelo considerando a inclusão da variável `PE_ratio`
se torna mais adequado, possuindo valores de AIC e BIC menores do que o modelo
sem PE_ratio.
...
A seguir teríamos o histórico das etapas do processo de seleção backward stepwise. No entanto, como nenhuma variável foi removida, apenas o cabeçalho é informado na saída. De fato, se removêssemos alguma das variáveis, o AIC aumentaria, o que não melhoraria o modelo.
...
Temos, a seguir, o modelo utilizando a técnica de stepwise backward.

```{r}
coeficientes <- backward$coefficients
coeficientes
```
...
Temos de maneira mais estruturada os passos tomados a seguir.
...
```{r}
coeficientes <- forward$coefficients
coeficientes
```
...
Para visualizar melhor os passos tomados, temos a tabela a seguir.
...
Segue, então, o modelo utilizando a técnica de stepwise both.

```{r}
coeficientes <- both$coefficients
coeficientes
```
